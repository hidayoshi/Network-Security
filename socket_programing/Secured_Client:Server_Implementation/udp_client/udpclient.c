...
int main(int argc, char *argv[])
{
  ...
  if ((numbytes = sendto(sockfd, &packet, sizeof(packet), 0, p->ai_addr, p->ai_addrlen)) == -1)
  {
  ...
  while (quitFlag)
  {
    ...
    case PASS_REQ: // Receive PASS_REQ packet from server. Send PASS_RESP_PACKET to
      packet.ctrl_msg.header = htons(3);
      // strcpy(packet.pass_resp.password, argv[2 + packetcou]);
      // fprintf(stderr, "%d password: %s\n", packetcou, packet.pass_resp.password);
      if (packetcou == 1)
      {
        strcpy(packet.pass_resp.password, passwd1);
      }
      else if (packetcou == 2)
      {
        strcpy(packet.pass_resp.password, passwd2);
      }
      else if (packetcou == 3)
      {
        strcpy(packet.pass_resp.password, passwd3);
      }

      if ((numbytes = sendto(sockfd, &packet, sizeof(packet), 0, p->ai_addr, p->ai_addrlen)) == -1)
      {
        perror("client: PASS_REQ");
        exit(1);
      }
      packetcou++;
      break;
    ...
    case DATA: // Receive DATA_PACKET from server. Get the data using strapp() function in helper_function.c. Write data to file (for exp. out.txt)
      strapp(&datamsg, packet.data.data);
      write_output(fileOut, datamsg);
      break;
    ...
    case TERMINATE: // Receive TERMINATE packet from server. Check the SHA1 digest
      ...
      result = SHA1_Update(&c, datamsg, strlen(datamsg));
      if (result == 1)
      {
      ...
      if (strcmp((const char *)sha1_hash, packet.terminate.digest) == 0)
      {
